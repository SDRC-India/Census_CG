...
public class HeaderFooter extends PdfPageEventHelper {
	...
	public void onStartPage(PdfWriter writer, Document document) {
		...
		try {
			image = Image.getInstance(context
					.getRealPath("resources\\images\\censuscg_header.png"));
			int indentation = 0;
	...
	public void onEndPage(PdfWriter writer, Document document) {
		...
		try {
			image = Image.getInstance(context
					.getRealPath("resources\\images\\censuscg_footer.png"));
			int indentation = 0;
			float scaler = ((document.getPageSize().getWidth() - indentation) / image
					.getWidth()) * 100;
			image.scalePercent(scaler);
		...
		String domainName = "from http://devserver.sdrc.co.in:8080/censuscg";
